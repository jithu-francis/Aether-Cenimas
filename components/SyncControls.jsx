  ...
  return (
    <div className="flex items-center gap-3 sm:gap-6 w-full sm:w-auto">
      ...
      {isSynced ? (
        ...
        <button
          onClick={onInitSync}
          className="flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-accent-blue/10 text-accent-blue border border-accent-blue/20 hover:bg-accent-blue/20 transition-all duration-300 font-black text-[10px] sm:text-xs uppercase tracking-widest glow-blue group flex-1 sm:flex-auto"
          id="sync-start-btn"
        >
      ...
      <div className="flex flex-col flex-shrink-0">
        <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5">
          ...
          <span className={`text-[9px] sm:text-[10px] font-black uppercase tracking-widest ${isSynced ? 'text-accent-blue' : 'text-slate-500'}`}>
            {isSynced ? "LIVE SYNC" : "OFFLINE"}
          </span>
        </div>
        <p className="text-[8px] sm:text-[9px] font-bold text-slate-600 uppercase tracking-tighter">
          {isSynced ? "Playback linked" : "Local playback"}
        </p>
      </div>
    </div>
  );
