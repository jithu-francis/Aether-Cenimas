...
export function useSync(playerRef) {
  ...
  const isSyncedRef = useRef(false); // Ref mirror to avoid stale closures
  ...
  useEffect(() => {
    ...
    socket.on("sync:state", (state) => {
      ...
      if (state.active && state.initiatedBy) {
        ...
        // Apply master time if we are not the initiator or if we just joined
  ...
  const sendPlay = useCallback(
    (time) => {
      if (!isSyncedRef.current || isRemoteAction.current) return;
      const socket = getSocket();
